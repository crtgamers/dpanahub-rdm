...
/**
 * Normaliza un código de idioma del sistema (ej: en-US, es-CL) al formato con guión; no cambia el caso.
 *
 * @param code - Código crudo (puede tener guión bajo).
 * @returns Código normalizado (trim, _ → -) o DEFAULT_LOCALE si vacío.
 */
...
/**
 * Resuelve el locale a uno soportado (es, es-CL, en), con fallback en cadena.
 * Prioridad: es-CL → es → en; es-AR → es; en-US → en; otro → DEFAULT_LOCALE.
 *
 * @param locale - Código de idioma (sistema o guardado).
...
/**
 * Carga los mensajes de un idioma (lazy load desde ./<locale>/common.json).
 * Si falla, intenta cargar DEFAULT_LOCALE.
 *
 * @param locale - Código soportado (es, es-CL, en).
 * @returns Objeto de mensajes (claves i18n → texto).
 */
