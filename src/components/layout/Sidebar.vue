    ...
    >
      ...
      <nav class="sidebar__nav">
        ...
        <div class="sidebar__nav-section">
          ...
          <button
            class="sidebar__nav-item"
            :aria-label="t('nav.romsetBuilder')"
            :title="isSidebarCollapsed ? t('nav.romsetBuilder') : ''"
            @click="$emit('open-romset-builder')"
          >
            <div class="sidebar__nav-icon">
              <Layers
                :size="20"
      ...
      <div class="sidebar__footer">
        <div
          ...
          :title="isSidebarCollapsed ? dbStatusLabel : ''"
        >
          <div
            class="sidebar__status-indicator"
            :class="dbStatusIndicatorClass"
          ></div>
          <transition name="sidebar-text">
            <span
              v-if="!isSidebarCollapsed"
...
import { Home, Star, DownloadCloud, Layers, Terminal, Settings } from 'lucide-vue-next';
import { getDatabaseStatus } from '@/services/api/catalog';
import type { CatalogSource } from '@/services/api/catalog';
import type { DatabaseStatus } from '@/services/api/catalog';
...
const props = defineProps({
  ...
  /** Fuente de catálogo activa (myrient | lolroms) o null en pantalla de inicio */
  activeSource: {
    type: String as () => CatalogSource | null,
    default: null,
  },
...
/** Estado de la base de datos para el indicador del footer: none | compressed | operational */
const databaseStatus = ref<DatabaseStatus>('none');

async function refreshDatabaseStatus(): Promise<void> {
  const sourceAtStart = props.activeSource;
  if (sourceAtStart) {
    const res = await getDatabaseStatus(sourceAtStart);
    // Solo aplicar si la fuente no cambió durante la petición (evitar condición de carrera)
    if (props.activeSource === sourceAtStart) {
      databaseStatus.value = res.success && res.data != null ? res.data : 'none';
    }
    return;
  }
  // Sin fuente activa: combinar estado de todas las fuentes (mejor disponible)
  const [myrient, lolroms, pleasuredome, myabandonware] = await Promise.all([
    getDatabaseStatus('myrient'),
    getDatabaseStatus('lolroms'),
    getDatabaseStatus('pleasuredome'),
    getDatabaseStatus('myabandonware'),
  ]);
  if (props.activeSource !== null) return; // El usuario ya seleccionó una fuente
  const statuses = [
    myrient.success && myrient.data ? myrient.data : 'none',
    lolroms.success && lolroms.data ? lolroms.data : 'none',
    pleasuredome.success && pleasuredome.data ? pleasuredome.data : 'none',
    myabandonware.success && myabandonware.data ? myabandonware.data : 'none',
  ];
  if (statuses.some(s => s === 'operational')) databaseStatus.value = 'operational';
  else if (statuses.some(s => s === 'compressed')) databaseStatus.value = 'compressed';
  else databaseStatus.value = 'none';
}

const dbStatusLabel = computed(() => {
  switch (databaseStatus.value) {
    case 'operational':
      return t('nav.databaseOperational');
    case 'compressed':
      return t('nav.databaseCompressed');
    default:
      return t('nav.databaseNotFound');
  }
});

const dbStatusIndicatorClass = computed(() => {
  switch (databaseStatus.value) {
    case 'operational':
      return 'sidebar__status-indicator--online';
    case 'compressed':
      return 'sidebar__status-indicator--warning';
    default:
      return 'sidebar__status-indicator--error';
  }
});
...
/** Indicador de cola: "99+" cuando hay más de 99 archivos, sino el número. */
const queueBadgeDisplay = computed(() =>
  props.activeDownloadCount > 99 ? '99+' : String(props.activeDownloadCount)
);
...
onMounted(() => {
  // refreshDatabaseStatus ya se ejecutó por el watch(activeSource, { immediate: true })
  statusPollInterval = setInterval(refreshDatabaseStatus, 5000);
});
onUnmounted(() => {
  document.removeEventListener('keydown', handleDrawerKeydown);
...
.sidebar__status-indicator--warning {
  background: var(--warning-color);
  box-shadow: 0 0 0.625rem var(--warning-color);
}

.sidebar__status-indicator--error {
  background: var(--danger-color);
  box-shadow: 0 0 0.625rem var(--danger-color);
}
