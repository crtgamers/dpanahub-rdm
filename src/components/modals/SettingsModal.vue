      ...
      >
        ...
        <div class="settings-body">
            ...
            <div class="setting-card vertical">
              <div class="setting-info">
                ...
                <p
                  ...
                  class="setting-hint"
                >
                  {{ t('settings.pendingVersion', { version: pendingUpdateVersion }) }}
                </p>
              </div>
              <div class="path-selector">
                <button
          ...
          <div class="settings-section">
            ...
            <div class="setting-card vertical">
              <div class="setting-info">
                <label>{{ t('settings.visualTheme') }}</label>
                <p>{{ t('settings.visualThemeHint') }}</p>
              </div>
              <div class="theme-picker">
                <button
                  type="button"
                  class="theme-option"
                  :class="{ active: visualTheme === 'glassmorphism' }"
                  :aria-label="t('settings.themeGlassmorphism')"
                  @click="$emit('set-visual-theme', 'glassmorphism')"
                >
                  ...
                  <span class="theme-option__label">{{ t('settings.themeGlassmorphism') }}</span>
                </button>
                <button
                  type="button"
                  class="theme-option"
                  :class="{ active: visualTheme === 'aero' }"
                  :aria-label="t('settings.themeAero')"
                  @click="$emit('set-visual-theme', 'aero')"
                >
                  <span class="theme-preview theme-preview--aero">
                    <span class="theme-preview__bar" />
                    <span class="theme-preview__card" />
                    <span class="theme-preview__card" />
                  </span>
                  <span class="theme-option__label">{{ t('settings.themeAero') }}</span>
                </button>
              </div>
            </div>
            <div class="setting-card toggle">
              <div class="setting-info">
                <label>{{ t('settings.darkLightMode') }}</label>
                <p>{{ t('settings.darkLightModeHint') }}</p>
              </div>
              <input
                type="checkbox"
                :checked="!isDarkMode"
                class="ios-switch"
                :aria-label="t('settings.darkLightMode')"
          ...
          <div class="settings-section">
            ...
            <div class="setting-card vertical credits-card">
              ...
              <ul class="credits-list">
                <li
                  v-for="key in creditsAcknowledgementKeys"
                  :key="key"
                >
                  {{ t('settingsExtra.' + key) }}
                </li>
              ...
              <p class="credits-donate">
                {{ t('settingsExtra.creditDonateIntro') }}
                <button
                  type="button"
                  class="credits-link"
                ...
                <br />
                {{ t('settingsExtra.creditDonateOutro') }}
              </p>
            </div>
          </div>
        </div>
...
const appVersionDisplay = computed(
  () => appVersion.value || (typeof __APP_VERSION__ !== 'undefined' ? __APP_VERSION__ : '…')
);
...
const creditsAcknowledgementKeys = [
  'creditMyrient',
  'creditErista',
  'creditLolroms',
  'creditPleasureDome',
...
const userDataPathLoading = ref(false);
/** Ruta a mostrar: siempre un texto (ruta, cargando o guión si falla). */
const userDataPathDisplay = computed(() => {
  if (userDataPath.value) return userDataPath.value;
  if (userDataPathLoading.value) return t('settingsExtra.loadingPath');
  return '—';
});
...
function setupUpdateListeners() {
  ...
  updateListenersCleanup = [
    ...
    window.api.on('update-downloaded', async () => {
      updateStatus.value = 'ready';
      updateStatusMessage.value = t('settings.updateReady');
      updateDownloadPercent.value = null;
      const pendingRes = await getPendingUpdateVersion();
      if (pendingRes.success && pendingRes.data) pendingUpdateVersion.value = pendingRes.data;
    }),
...
async function loadUserDataPath() {
  userDataPathLoading.value = true;
  try {
    const res = await getUserDataPath();
    const path =
      (res as { path?: string }).path ?? (res as { data?: { path?: string } }).data?.path;
    if (res.success && path) userDataPath.value = path;
  } finally {
    userDataPathLoading.value = false;
  }
}

async function loadUpdaterState() {
  const res = await isUpdaterEnabled();
  updaterEnabled.value = res.success && res.data === true;
  if (updaterEnabled.value) {
    await loadAppVersion();
    setupUpdateListeners();
    const pendingRes = await getPendingUpdateVersion();
    if (pendingRes.success && pendingRes.data) pendingUpdateVersion.value = pendingRes.data;
    else pendingUpdateVersion.value = null;
  } else {
...
watch(
  ...
  async val => {
    if (val) {
      await loadUserDataPath();
      if (!userDataPath.value) {
        setTimeout(loadUserDataPath, 300);
      }
...
async function openProgramFolder() {
  ...
  if (res.success && !userDataPath.value) {
    const path =
      (res as { path?: string }).path ?? (res as { data?: { path?: string } }).data?.path;
    if (path) userDataPath.value = path;
  }
  if (!res.success && res.error) {
    logger.error('Error abriendo carpeta del programa:', res.error);
  }
}
...
async function openDonateLink() {
  ...
  if (!res.success && res.error) {
    logger.error('Error abriendo enlace de donación:', res.error);
  }
}
</script>
