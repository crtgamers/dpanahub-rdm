  ...
  >
    ...
    <div class="home-screen__grid">
      ...
      <button
        type="button"
        class="home-source-card"
        :disabled="loadingSource === 'pleasuredome'"
        :aria-busy="loadingSource === 'pleasuredome'"
        :aria-label="t('home.pleasuredome')"
        @click="$emit('select-source', 'pleasuredome')"
      >
        <div
          class="home-source-card__icon"
          aria-hidden="true"
        ...
        <span class="home-source-card__label">{{ t('home.pleasuredome') }}</span>
        <span
          v-if="loadingSource === 'pleasuredome'"
          class="home-source-card__loading"
        >
          <Loader2
            class="home-source-card__spinner"
      ...
      <button
        type="button"
        class="home-source-card"
        :disabled="loadingSource === 'myabandonware'"
        :aria-busy="loadingSource === 'myabandonware'"
        :aria-label="t('home.myabandonware')"
        @click="$emit('select-source', 'myabandonware')"
      >
        <div
          class="home-source-card__icon"
          aria-hidden="true"
        ...
        <span class="home-source-card__label">{{ t('home.myabandonware') }}</span>
        <span
          v-if="loadingSource === 'myabandonware'"
          class="home-source-card__loading"
        >
          <Loader2
            class="home-source-card__spinner"
...
/**
 * HomeScreen - Pantalla de inicio con las fuentes de catálogo: Myrient, LoLROMs, PleasureDome y My Abandonware.
...
defineProps<{
  loadingSource?: 'myrient' | 'lolroms' | 'pleasuredome' | 'myabandonware' | null;
}>();

defineEmits<{
  (_e: 'select-source', _source: 'myrient' | 'lolroms' | 'pleasuredome' | 'myabandonware'): void;
}>();
