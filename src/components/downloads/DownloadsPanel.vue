  ...
  >
    <div class="downloads-header">
      ...
      <div class="header-actions">
        ...
        <!-- Limpiar solo completadas: visible cuando hay más de 1 completada -->
        <button
          v-if="completedCount > 1"
          class="btn-action-header btn-clear-completed glass-effect"
          :title="t('downloadsPanel.clearCompletedTitle')"
          @click="$emit('clear-completed-from-list')"
        >
          <Trash2 :size="18" />
          <span>{{ t('downloadsPanel.clearCompleted') }}</span>
        </button>
...
/** Número de descargas en estado completado (para mostrar botón "Limpiar completadas"). */
const completedCount = computed(
  () =>
    (props.downloads as DownloadItemRecord[]).filter(
      d => ((d.state as string) || '').toLowerCase() === 'completed'
    ).length
);
