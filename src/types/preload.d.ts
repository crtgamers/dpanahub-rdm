...
export interface PreloadApi {
  ...
  loadDatabase(
    _source: 'myrient' | 'lolroms' | 'pleasuredome' | 'myabandonware'
  ): Promise<IpcResponse<unknown>>;
  closeDatabase(): Promise<IpcResponse<unknown>>;
  getCurrentSource(): Promise<
    IpcResponse<'myrient' | 'lolroms' | 'pleasuredome' | 'myabandonware' | null>
  >;
  getDatabaseStatus(
    _source: 'myrient' | 'lolroms' | 'pleasuredome' | 'myabandonware'
  ): Promise<IpcResponse<'none' | 'compressed' | 'operational'>>;
  ...
  clearNavCache(): Promise<IpcResponse<unknown>>;
  ...
  >;
  ensureDownloadEngine(): Promise<IpcResponse<{ initialized: boolean }>>;
  ...
  clearCompletedFromList(): Promise<IpcResponse<unknown>>;
  ...
  ): Promise<IpcResponse<{ path: string }> & { path?: string }>;
  validateDownloadPath(_filePath: string): Promise<
    IpcResponse<unknown> & {
      path?: string;
  ...
  getPendingUpdateVersion(): Promise<IpcResponse<string | null>>;
  isUpdaterEnabled(): Promise<IpcResponse<boolean>>;

  romsetBuilderSummary(_params: {
    folderIds: number[];
  }): Promise<IpcResponse<{ fileCount: number; totalSizeBytes: number }>>;
