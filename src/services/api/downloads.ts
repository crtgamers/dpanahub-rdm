/**
 * API de descargas: delegación al proceso main vía window.api.
 *
 * Expone: downloadFolder, cleanHistory, clearHistory, applyDownloadSettings, getDownloadState,
 * addDownload, pause/resume/cancel/delete por ítem o global, getSuggestedTestFile, runConnectionTest,
 * y suscripciones a eventos (onDownloadStateChanged, onDownloadProgress, onDownloadCompleted, etc.).
 * Todas las funciones comprueban getApi() y normalizan respuestas con success/error.
 *
...
/**
 * Quita de la lista solo las descargas en estado completado (no borra archivos en disco).
 * Útil para hacer espacio antes de agregar muchas descargas manuales.
 * @returns success, count de ítems eliminados de la lista y error opcional.
 */
export async function clearCompletedFromList(): Promise<CleanHistoryResponse> {
  const api = getApi();
  if (!api) return { success: false, count: 0, error: API_ERRORS.NOT_AVAILABLE };
  try {
    const result = await api.clearCompletedFromList();
    const ok = result?.success === true;
    const res = result as { count?: number; data?: { count?: number } };
    const count = res?.count ?? res?.data?.count ?? 0;
    return {
    ...
    apiLogger.error('Error eliminando completadas de la lista:', error);
    return { success: false, count: 0, error: (error as Error).message || GENERAL_ERRORS.UNKNOWN };
  }
}
 ...
 * @param downloadId - ID de la descarga en la cola.
 * @returns success y data o error.
 */
