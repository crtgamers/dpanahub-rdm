...
export interface LoggerInstance extends ScopedLogger {
  ...
  /** Desuscribe el listener IPC 'backend-log'. Llamar en onUnmounted. */
  teardown?: () => void;
}
...
const logger: LoggerInstance = {
  ...
  /** Desuscribe el listener IPC 'backend-log'. Llamar en onUnmounted para evitar retención. */
  teardown: teardownLogger,
};

/**
 * Desuscribe el listener IPC de logs del backend (evita acumulación de listeners en hot-reload o múltiples ventanas).
 */
export function teardownLogger(): void {
  if (typeof backendLogUnsubscribe === 'function') {
