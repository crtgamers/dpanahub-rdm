...
export interface UseAppLayoutReturn {
  ...
  /** Romset Builder wizard visible */
  showRomsetBuilder: Ref<boolean>;
  ...
  openStatistics: () => void;
  /** Abre el wizard Romset Builder y cierra sidebar */
  openRomsetBuilder: () => void;
}
...
export function useAppLayout(options: UseAppLayoutOptions): UseAppLayoutReturn {
  ...
  const showRomsetBuilder = ref(false);
  ...
  const openRomsetBuilder = (): void => {
    sidebarDrawerOpen.value = false;
    showRomsetBuilder.value = true;
  };
