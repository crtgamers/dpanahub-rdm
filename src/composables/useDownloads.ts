/**
 * Composable para la cola de descargas y su sincronización con el backend.
 *
 * Expone el estado de la cola (downloads, stateVersion), suscripciones a eventos IPC
 * (download-state-changed, download-progress, download-completed, etc.) y acciones:
 * addDownload, downloadFolder, pause/resume/cancel/delete por ítem o global, confirmOverwrite.
 * Ordena y filtra según preferencias (useSettings). No orquesta modales; eso se hace en App.vue.
 *
 * @module useDownloads
 */
...
import { getMemoryStats, formatMemoryStats } from '../services/api';
 ...
 *
 * @returns Objeto con downloads, allDownloads, downloadQueue, pendingConfirmations, acciones de cola, ordenación, initDownloads y cleanup.
 */
export function useDownloads(): {
  ...
  const {
    ...
    selectDownloadFolder: _selectDownloadFolder,
    downloadsSortBy,
  ...
  /** Resultado cuando no hay carpeta configurada: el caller debe mostrar el modal de "no download folder". */
  const ensureDownloadPath = async (): Promise<
    { ok: true } | { ok: false; error: string } | { ok: false; showNoFolderModal: true }
  > => {
    if (
      !downloadPath.value ||
      (typeof downloadPath.value === 'string' && downloadPath.value.trim().length === 0)
    ) {
      log.info('No hay ruta de descarga configurada; se debe mostrar modal de selección.');
      return { ok: false, showNoFolderModal: true };
    }
    return { ok: true };
  };
  ...
  const download = async (
    ...
    try {
      ...
      if (!pathCheck.ok) {
        if ('showNoFolderModal' in pathCheck && pathCheck.showNoFolderModal) {
          return { success: false, showNoFolderModal: true, file };
        }
        return { success: false, error: (pathCheck as { error?: string }).error };
      }
  ...
  > => {
    ...
    if (!pathCheck.ok) {
      if ('showNoFolderModal' in pathCheck && pathCheck.showNoFolderModal) {
        return { success: false, showNoFolderModal: true, folderParams };
      }
      return { success: false, error: (pathCheck as { error?: string }).error };
    }
  ...
  const clearDownloads = async (): Promise<void> => {
    ...
    if (!api) return;
    try {
      const memBefore = await getMemoryStats();
      console.log('[Mem] Antes de borrar cola:', formatMemoryStats(memBefore));
      ...
      const memAfter = await getMemoryStats();
      console.log('[Mem] Después de borrar cola:', formatMemoryStats(memAfter));
    } catch (e) {
  ...
  const initDownloads = async (): Promise<void> => {
    ...
    try {
      // Asegurar inicialización perezosa del motor de descargas en el main
      if (typeof api.ensureDownloadEngine === 'function') {
        await api.ensureDownloadEngine();
      }
      const result = await api.getDownloadState();
      if (result?.success && result.data) applySnapshot(result.data);
    } catch (e) {
