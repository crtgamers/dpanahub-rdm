/**
 * Composable para la gestión de configuración de la aplicación.
 *
 * Centraliza: ruta de descargas, preferencias de cola (simultáneas, chunks, umbral de confirmación),
 * tema (claro/oscuro), idioma, preferencias de movimiento y tema visual (glassmorphism / aero).
 * Persiste en JSON vía `window.api` (readConfigFile, writeConfigFile) y sincroniza con el motor
 * de descargas (applyDownloadSettings).
 *
 * @module useSettings
 */
...
export type VisualThemeKey = 'glassmorphism' | 'aero';

export const VISUAL_THEMES: Record<VisualThemeKey, { bodyClass: string }> = {
  glassmorphism: { bodyClass: '' },
  aero: { bodyClass: 'aero-theme' },
};
...
const visualTheme = ref<VisualThemeKey>('glassmorphism');
...
interface UIPreferencesData {
  ...
  /** Tema visual: glassmorphism (default) o aero (Frutiger Aero) */
  visualTheme?: VisualThemeKey;
}

/**
 * Composable de configuración: refs reactivos y métodos para cargar/guardar en disco.
 *
 * @returns Objeto con refs (downloadPath, maxParallelDownloads, isDarkMode, locale, visualTheme, …),
 *   métodos de persistencia (loadDownloadSettings, saveDownloadSettings, loadUIPreferences, saveUIPreferences),
 *   initSettings, selectDownloadFolder, toggleTheme, setLocale y setters de tema/color/motion.
 */
export function useSettings(): {
  ...
  visualTheme: Ref<VisualThemeKey>;
  setVisualTheme: (_theme: VisualThemeKey) => void;
  ...
  const loadUIPreferences = async (): Promise<void> => {
    try {
      ...
      if (result.success && result.data) {
        ...
        if (data.visualTheme && data.visualTheme in VISUAL_THEMES) {
          visualTheme.value = data.visualTheme;
        }
      }
      updateThemeClass();
      updateVisualThemeClass();
      updatePrimaryColor();
      updateMotionClass();
  ...
  const selectDownloadFolder = async (): Promise<void> => {
    try {
      ...
      showToast({ title: t('errors.selectDownloadFolder'), type: 'error' });
    }
  };
  ...
  const updateVisualThemeClass = (): void => {
    if (typeof document === 'undefined') return;
    const body = document.body;
    Object.values(VISUAL_THEMES).forEach(t => {
      if (t.bodyClass) body.classList.remove(t.bodyClass);
    });
    const cfg = VISUAL_THEMES[visualTheme.value];
    if (cfg?.bodyClass) body.classList.add(cfg.bodyClass);
  };

  const setVisualTheme = (theme: VisualThemeKey): void => {
    if (!(theme in VISUAL_THEMES)) return;
    startThemeTransition();
  ...
  /** Convierte hex #RRGGBB a string "r, g, b" para usar en rgba(var(--primary-color-rgb), alpha) */
