/**
 * Composable para búsqueda en el catálogo (Myrient / LoLROMs).
 *
 * Expone: término de búsqueda (searchTerm), resultados (searchResults), estado (isSearching, searchError),
 * ordenación (sortField, sortDirection), searchFolders/searchFiles (memoizados), totalResults,
 * search(), searchWithDebounce(), clearSearch(), executeSearch(), setSortField, cleanup.
 * Opcional: getScopeFolderId / getScopeFolderIds para limitar búsqueda a carpeta(s).
 *
 * @module useSearch
 */
 ...
 *
 * @param options - Opcional: getScopeFolderId (buscar en esta carpeta) y/o getScopeFolderIds (limitar a varias carpetas).
 * @returns Objeto con refs (searchTerm, searchResults, isSearching, isSearchCancelled, searchError, sortField, sortDirection),
 *   computed (searchFolders, searchFiles, totalResults, hasSearchResults), search(), searchWithDebounce(), clearSearch(),
 *   executeSearch(), setSortField(), setSortDirection(), cleanup().
 */
