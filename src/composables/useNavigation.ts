/**
 * Composable para navegación por carpetas en el catálogo (Myrient / LoLROMs).
 *
 * Expone: nodo actual (currentNodeId, currentParentId), hijos (allChildren, folders, files),
 * breadcrumb (breadcrumbPath, locationPath), historial (navigationHistory, goBack, goBackInHistory),
 * loadChildren (en paralelo getChildren + getNodeInfo cuando no es raíz), loadMoreChildren,
 * navigateToNode, navigateToId, goToRoot, initNavigation, collapseToInitialPage.
 *
 ...
 *
 * @returns Objeto con refs (currentNodeId, allChildren, breadcrumbPath, statusMessage, navigationHistory, …),
 *   computed (folders, files, locationPath, isAtRoot, canGoBack, hasMoreChildren) y funciones
 *   (loadChildren, loadBreadcrumb, loadMoreChildren, navigateToNode, navigateToId, goToRoot, goBack, goBackInHistory, clearHistory, initNavigation, collapseToInitialPage).
 */
export function useNavigation(): {
  ...
  const loadBreadcrumb = async (): Promise<void> => {
    ...
    try {
      ...
      statusMessageKey.value = 'errors.loadContent';
      statusMessageParams.value = {};
    }
  };
  ...
  const goToRoot = async (): Promise<void> => {
    if (currentNodeId.value !== 1) {
      ...
      console.log('[Mem] Antes de volver a inicio:', formatMemoryStats(memBefore));
      navigationHistory.value.push(currentNodeId.value);
      // Liberar caché de navegación en main para reducir RAM al volver al inicio
      void clearNavCache();
    }
    currentNodeId.value = 1;
    await loadChildren();
    const memAfter = await getMemoryStats();
    console.log('[Mem] Después de volver a inicio:', formatMemoryStats(memAfter));
  };
  ...
  /**
   * Vacía el estado de navegación sin cargar datos (para cuando se vuelve a la pantalla de inicio
   * y se cierra el catálogo). Libera referencias a listas grandes en el renderer.
   */
