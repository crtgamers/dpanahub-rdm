# Dpana Hub – ROM Download Manager - Deploy Script
...
$TEMP_PATH = Join-Path $env:TEMP "DpanaHubRDM_Deploy_$(Get-Random)"
...
Write-Header "Instalador Dpana Hub – ROM Download Manager"

# Configurar ruta de instalacion
$INSTALL_PATH = Join-Path $env:USERPROFILE "Dpana Hub"
...
$ZipFile = Join-Path $TEMP_PATH "dpanahub-rdm-complete.zip"
...
# Mover opcionalmente otras bases de catálogo (lolrom, pleasuredome, myabandonware) si están en raíz
$OptionalDbNames = @("lolrom_data.db", "pleasuredome_data.db", "myabandonware_data.db")
$ResourcesPathDeploy = Join-Path $INSTALL_PATH "resources"
foreach ($dbName in $OptionalDbNames) {
    $srcPath = Join-Path $INSTALL_PATH $dbName
    if (Test-Path $srcPath) {
        if (-not (Test-Path $ResourcesPathDeploy)) { New-Item -ItemType Directory -Path $ResourcesPathDeploy -Force | Out-Null }
        $destPath = Join-Path $ResourcesPathDeploy $dbName
        Move-Item -Path $srcPath -Destination $destPath -Force -ErrorAction SilentlyContinue
        if (Test-Path $destPath) { Write-Host "   $dbName movido a resources/" -ForegroundColor Green }
    }
}
...
try {
    ...
    $ShortcutPath = Join-Path $DesktopPath "Dpana Hub – ROM Download Manager.lnk"
    
    $Shortcut = $WshShell.CreateShortcut($ShortcutPath)
    $Shortcut.TargetPath = $MainExe.FullName
    $Shortcut.WorkingDirectory = $INSTALL_PATH
    $Shortcut.Description = "Dpana Hub – ROM Download Manager - Descargador de ROMs"
    $Shortcut.Save()
...
Write-Status "La aplicación Dpana Hub – ROM Download Manager ha sido instalada exitosamente." "Success"
Write-Host ""
...
Write-Status "Iniciando Dpana Hub – ROM Download Manager..." "Info"
