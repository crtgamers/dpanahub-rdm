...
describe('StateStore', () => {
  ...
  describe('con DB (integration)', () => {
    ...
    it('permite starting -> queued para reintentos tras error transitorio', () => {
      if (!initOk) return;
      store.addDownload({ id: 2003, title: 'retry.zip', totalBytes: 0 });
      expect(store.transitionState(2003, DownloadState.STARTING)).toBe(true);
      expect(store.canTransition(DownloadState.STARTING, DownloadState.QUEUED)).toBe(true);
      expect(store.transitionState(2003, DownloadState.QUEUED)).toBe(true);
      expect(store.getDownload(2003)!.state).toBe(DownloadState.QUEUED);
    });
