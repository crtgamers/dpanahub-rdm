# Dpana Hub – ROM Download Manager - Script de Instalación Automática
# Este script descarga la aplicación desde Google Drive y la instala

param(
    [string]$InstallPath = "$env:LOCALAPPDATA\Dpana Hub",
...
# Carpeta temporal
$TempPath = Join-Path $env:TEMP "DpanaHubRDM_Install_$(Get-Random)"
$ZipFile = Join-Path $TempPath "dpanahub-rdm-complete.zip"

Write-Header "Instalador de Dpana Hub – ROM Download Manager"
...
if ($DbArchive) {
    $AppResourcePath = Join-Path $env:LOCALAPPDATA "Dpana Hub" "resources"
...
Write-Status "La aplicación Dpana Hub – ROM Download Manager ha sido instalada correctamente." "Success"
Write-Host ""
