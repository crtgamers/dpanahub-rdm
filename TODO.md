...
## Dpana Hub – ROM Download Manager – Especificación Detallada

**Proyecto:** Dpana Hub – ROM Download Manager  
...
- Cobertura pendiente en: DownloadEngine (flujos principales), ChunkManager, SimpleDownloader, ChunkDownloader, EventBus; servicios (DownloadService, FileService, QueueService, SearchService); ipcStateHandlers (integración con mock del engine).
- En **validation.ts**: tests para `normalizePriority`, `validateDownloadParams`, `validateDownloadFolderParams`, `validateAndPrepareDownloadParams`.

### JSDoc

- CONTRIBUTING exige JSDoc en funciones públicas. Revisar módulos que aún tengan funciones sin documentar.

### Refactors a medio plazo

- Clases monolíticas: DownloadEngine (~2000 L), StateStore (~1310 L).
- Componentes Vue muy grandes: SettingsModal, FiltersPanel, DownloadsPanel.
- División de handlers IPC por dominio (carpeta `electron/ipc/` con catalog, config, window, logging).
