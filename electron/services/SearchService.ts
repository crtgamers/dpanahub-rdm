...
/** Información de paginación: total, limit, offset, currentPage, totalPages, hasNext/hasPrevious, nextOffset/previousOffset. */
...
/** Entrada de caché: result, timestamp, lastAccess, accessCount (para LFU). */
...
/** Opciones de búsqueda normalizadas (límites y flags dentro de rangos válidos). */
...
export default class SearchService extends BaseService {
  ...
  /** Caché en memoria (cacheMaxSize 100, cacheTTL 5 min, LFU al llenar). */
  constructor() {
    super('SearchService');
    this.cache = new Map();
    // Limitar caché de resultados en memoria para reducir uso de RAM
   ...
   * @returns { valid: true, data, original } | { valid: false, error? } | ServiceResponse.
   */
  ...
  /** Trim, colapsa espacios y limita a MAX_SEARCH_TERM_LENGTH. */
   ...
   * @param options - Opciones crudas del frontend.
   * @returns NormalizedSearchOptions.
   */
  ...
  /** Devuelve 'fts' o 'like' según longitud del término y opciones (usePhrase, useOR). */
  ...
  /** Prepara el término para FTS: frase entre comillas, OR entre palabras o prefijo (*). */
   ...
   * @param offset - Desplazamiento actual.
  ...
  /** Clave de caché a partir de término y opciones normalizados (para getFromCache/setCache). */
  ...
  /** Obtiene resultado del caché por clave; null si no existe o expiró (actualiza lastAccess y accessCount). */
  ...
  /** Obtiene resultado del caché por término y opciones (genera clave internamente). */
  ...
  /** Guarda resultado en caché por clave; si está lleno elimina entrada LFU. No guarda si result.success === false. */
  ...
  /** Elimina entradas con edad > cacheTTL. @returns Número de entradas eliminadas. */
  ...
  /** Vacía el caché. @returns Número de entradas eliminadas. */
  clearCache(): number {
    try {
  ...
  /** Estadísticas del caché: size, maxSize, ttl, expiredEntries, averageAge, hitRate, hits, misses. */
  getCacheStats(): {
    size: number;
    maxSize: number;
    ttl: number;
