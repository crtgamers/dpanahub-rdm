...
/** Resultado de checkAvailability: slots, si puede iniciar, si debe encolar, límites. */
...
/** Estadísticas de cola: total, active, slotsAvailable, byPriority, canStart, shouldQueue. */
...
export default class QueueService extends BaseService {
  ...
  /** Inicializa con config.downloads (maxConcurrent, queueOrdering: aging, sjf, retryPenalty). */
  constructor() {
    super('QueueService');
  ...
  /** Prioridad efectiva: base + aging (tiempo en cola) - penalización por reintentos. */
  ...
  /** Ordena la cola por prioridad efectiva, SJF (si está habilitado) y fecha de creación. */
  ...
  /** Devuelve los primeros N elementos de la cola ordenada, con N = slots disponibles (maxConcurrent - activeCount). */
  ...
  /** Índice (0-based) donde quedaría una descarga con la prioridad dada en la cola ordenada. */
  ...
  /** Comprueba slots disponibles, si puede iniciar y si debe encolar según maxQueueSize. */
  ...
  /** Estadísticas agregadas de la cola (total, active, slotsAvailable, byPriority, canStart, shouldQueue). */
  ...
  /** Cambia la prioridad de una descarga en la cola y devuelve la cola reordenada y nueva posición. */
  prioritizeDownload(
  ...
  /** Mueve una descarga a una nueva posición en la cola y devuelve la cola reordenada. */
  reorderDownload(
    downloadId: string | number,
    queue: DownloadLike[],
    newPosition: number
  ...
  /** Suma de velocidades (bytes/s) de las descargas activas; 0 si ninguna tiene velocidad. */
  ...
  /** Estima segundos/minutos/horas hasta que una descarga empiece (posición en cola y velocidad). */
  ...
  /** Estima tiempo total para completar todas las descargas en cola (bytes totales / velocidad efectiva). */
  estimateQueueTime(
    queue: DownloadLike[] | null | undefined,
    activeCount: number,
    averageSpeedBytesPerSec = 0
