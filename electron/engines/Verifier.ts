...
export default class Verifier {
  ...
  /**
   * Comprueba tamaño del archivo y opcionalmente hash SHA-256 (si expectedHash existe o archivo ≥1 MB).
   *
   * @param filePath - Ruta absoluta del archivo.
   * @param expectedSize - Tamaño esperado en bytes.
   * @param expectedHash - Hash SHA-256 esperado (opcional); si no se pasa pero el archivo es ≥1 MB se calcula hash.
   * @param onProgress - Callback opcional de progreso (0–1) durante el cálculo de hash.
  ...
  /**
   * Calcula el hash SHA-256 del archivo (lectura por bloques con callback de progreso).
   * @param filePath - Ruta absoluta del archivo.
   * @param onProgress - Callback opcional (progreso 0–1).
   * @returns Hash en hexadecimal (minúsculas).
   */
  async calculateHash(
    filePath: string,
    onProgress: ((_progress: number) => void) | null = null
  ...
  /**
   * Verifica un chunk individual: tamaño y opcionalmente hash.
   * @param chunkPath - Ruta del archivo temporal del chunk.
   * @param expectedSize - Tamaño esperado en bytes.
   * @param expectedHash - Hash SHA-256 esperado (opcional).
   * @returns VerifyFileResult (valid, sizeValid, hashValid, error).
   */
  async verifyChunk(
    chunkPath: string,
    expectedSize: number,
  ...
  /**
   * Verifica varios chunks en lotes paralelos.
   * @param chunks - Lista de { path, expectedSize, expectedHash? }.
   * @param maxConcurrent - Máximo de verificaciones simultáneas (por defecto 4).
   * @returns Array de VerifyFileResult en el mismo orden que chunks.
   */
