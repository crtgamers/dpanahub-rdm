...
export class SpeedTracker {
  ...
  /**
   * Asegura que la descarga esté siendo trackeada; si no lo está, llama a startTracking.
   * Si se reanuda una descarga, pasar initialDownloadedBytes para evitar un pico de velocidad en el primer cálculo.
   *
   * @param downloadId - ID de la descarga.
   * @param sessionStartTime - Timestamp de inicio de sesión (por defecto ahora).
   * @param initialDownloadedBytes - Bytes ya descargados (p. ej. al reanudar desde .part).
   */
  ensureTracking(
    downloadId: number,
  ...
  /**
   * Inicia el seguimiento de velocidad y ETA para una descarga.
   * @param downloadId - ID de la descarga.
   * @param sessionStartTime - Timestamp de inicio (por defecto ahora).
   * @param initialDownloadedBytes - Bytes ya descargados al iniciar (reanudación).
   */
  startTracking(
    downloadId: number,
    sessionStartTime = Date.now(),
  ...
  /**
   * Actualiza métricas con el progreso actual y devuelve velocidad (EMA) y tiempo restante.
   * @param downloadId - ID de la descarga.
   * @param downloadedBytes - Bytes descargados hasta ahora.
   * @param totalBytes - Tamaño total del archivo (0 si desconocido).
   * @returns SpeedUpdateResult (speedBytesPerSec, remainingTime) o null si no hay tracker.
   */
  update(
    downloadId: number,
  ...
  /** Deja de trackear la descarga (elimina la entrada). */
  stopTracking(downloadId: number): void {
    this.trackers.delete(downloadId);
  }

  /** Elimina todas las entradas de seguimiento. */
  clear(): void {
    this.trackers.clear();
  }
}
