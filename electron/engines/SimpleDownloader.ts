...
interface ActiveSimpleEntry {
  ...
  /** Intervalo de detección de inactividad (timeout de inactividad). */
...
/**
 * Guarda checkpoint de integridad parcial (hash de últimos N bytes del .part) para reanudar después.
 * Debe llamarse antes de pausar una descarga simple (cierre del stream).
 *
 * @param engine - Referencia al motor (stateStore, activeDownloads).
...
/**
 * Maneja la respuesta HTTP de una descarga simple: enlaza data/end/error, escribe al stream de disco,
 * actualiza progreso y estado (completado/fallido) vía stateStore y eventBus.
 *
 * @param engine - Referencia al motor (stateStore, eventBus, activeDownloads).
 * @param downloadId - ID de la descarga.
 * @param response - Objeto respuesta (statusCode, headers, on, destroy, pause, resume).
 * @param _request - Request original (no usado; mantiene firma para compatibilidad).
 */
export async function handleSimpleResponse(
  ...
  // Red de seguridad (backpressure): si el fileStream se cierra inesperadamente, detener la response.
  ...
  // Timeout de inactividad: detecta descargas simples sin progreso (stalled).
  const idleTimeout =
  ...
  const idleCheckInterval = setInterval(() => {
    ...
    if (downloadedBytes > lastIdleCheckBytes) {
      ...
      log.warn(
        `Descarga ${downloadId}: sin progreso por ${timeoutSec}s (timeout de inactividad), abortando`
      );
      ...
      if (isTransientError && retryCount < maxRetries) {
        // El timeout de inactividad se clasifica como 'timeout' → retry rápido
        const idleError = new Error(
          `Timeout de inactividad: sin progreso por ${timeoutSec}s`
        ) as Error & {
          code?: string;
        };
        ...
        log.info(
          `Descarga ${downloadId}: reintentando tras timeout de inactividad en ${Math.round(adaptiveDelay / 1000)}s (${retryCount + 1}/${maxRetries})`
        );
        stateStore.updateDownload(downloadId, { retryCount: retryCount + 1 });
        setTimeout(() => {
          stateStore.transitionState(downloadId, DownloadState.QUEUED);
          engine.processQueue().catch(e => {
            log.error(`Error reintentando descarga ${downloadId} tras timeout de inactividad:`, e);
          });
        }, adaptiveDelay);
        engine._cleanupActiveDownload(downloadId);
      } else {
        stateStore.transitionState(downloadId, DownloadState.FAILED);
        stateStore.updateDownload(downloadId, {
          lastError: `Sin progreso por ${timeoutSec}s (timeout de inactividad)`,
        });
        eventBus.emitDownloadFailed(
          downloadId,
          new Error(`Sin progreso por ${timeoutSec}s (timeout de inactividad)`)
        );
        engine._cleanupActiveDownload(downloadId);
      }
    }
