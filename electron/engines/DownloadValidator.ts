 ...
 * @param savePath - Ruta donde se guardará el archivo.
 * @param downloadPath - Directorio de descargas permitido (opcional); si se pasa, savePath debe estar dentro.
 * @returns ValidateSavePathResult con valid, error y sanitizedPath si válido.
 */
...
/**
 * Indica si el error es de red transitorio (reintento razonable: ECONNRESET, ETIMEDOUT, net::ERR_*, etc.).
 * @param error - Error capturado (puede tener code y message).
 * @returns true si el error se considera transitorio.
 */
 ...
 * @param retryAfter - Valor de la cabecera Retry-After.
 * @param opts - retryAfter429DefaultMs, retryAfter429MaxMs para acotar el delay.
 * @returns Delay en milisegundos o null si no se pudo parsear.
 */
...
/**
 * Calcula delay de reintento en ms (backoff exponencial con jitter, acotado por config).
 * @param retryCount - Número de reintentos ya realizados (0 = primer reintento).
 ...
 * - `unknown`: error no clasificado; se usa backoff genérico.
 */
...
/**
 * Clasifica un error de red transitorio en una categoría específica (timeout, connection_reset, dns, etc.).
 * Útil para adaptar la estrategia de retry según el tipo de fallo.
 * @param error - Error capturado (code y message).
 * @returns TransientErrorType: timeout, connection_reset, connection_refused, dns, network_change, server_overload, pipe_broken, unknown.
 */
export function classifyTransientError(error: Error & { code?: string }): TransientErrorType {
  ...
  // Tipo timeout: servidor lento pero alcanzable
  if (
    code === 'ETIMEDOUT' ||
...
function getRetryProfiles(): Record<TransientErrorType, RetryProfile> {
  ...
  const defaults: Record<TransientErrorType, RetryProfile> = {
    // Tipo timeout: servidor lento pero vivo → retry rápido
    timeout: { baseDelayMs: 5_000, maxDelayMs: 20_000, growthFactor: 1.5, jitterFactor: 0.2 },
    // Reset de conexión: disrupción de red → backoff moderado
    connection_reset: {
      baseDelayMs: 10_000,
      maxDelayMs: 60_000,
      growthFactor: 2,
    ...
    // Sobrecarga del servidor: Retry-After tiene prioridad; este perfil es respaldo
    server_overload: {
      baseDelayMs: 30_000,
      maxDelayMs: 300_000,
      growthFactor: 2,
...
export function calculateAdaptiveRetryDelay(
  ...
  // Retry-After explícito (429/503) tiene prioridad
