...
import { getDownloadWorkerPath } from '../utils/workerPaths';
...
import { selectDownloadsToStart } from './QueueProcessor';
...
/** Límite de descargas en snapshots devueltos por el motor (evita cargar toda la cola en memoria). */
const SNAPSHOT_LIMIT = 500;
/** Límite para operaciones bulk (pauseAll/cancelAll/resumeAll) que necesitan lista filtrada. */
const SNAPSHOT_LIMIT_BULK = 2000;
...
export class DownloadEngine {
  ...
  /** IDs para los que ya se disparó startDownload pero aún no han pasado a STARTING (evita doble arranque). */
  private _downloadIdsPendingStart = new Set<number>();
  ...
  async processQueue(): Promise<void> {
    ...
    try {
      ...
      const { toStart, queuedCount, slotsAvailable } = selectDownloadsToStart(
        this.stateStore,
        this.scheduler,
        this.concurrencyController,
        { maxQueueBatchSize, excludeIds: this._downloadIdsPendingStart }
      );

      if (queuedCount === 0) return;
      log.info(`[processQueue] Procesando ${queuedCount} descarga(s) en cola`);

      if (!app.isPackaged) {
        const globalActiveCount = this.concurrencyController.getGlobalActiveCount();
        log.debug(
          `[processQueue] Estado: ${queuedCount} en cola, ${globalActiveCount} activas (slots libres: ${slotsAvailable})`
        );
      }

      if (toStart.length === 0) return;

      for (const { id: downloadId } of toStart) {
        ...
        this._downloadIdsPendingStart.add(downloadId);
        // No hacer await: así processQueue libera isProcessing; la descarga sigue en cola hasta
        // que startDownload pase a STARTING. excludeIds evita que la siguiente pasada la vuelva a elegir.
        this.startDownload(downloadId).catch(error => {
          ...
          const current = this.stateStore.getDownload(downloadId);
          if (
            current?.state === DownloadState.STARTING ||
            current?.state === DownloadState.QUEUED
          ) {
            this.stateStore.transitionState(downloadId, DownloadState.FAILED, current.state);
      ...
      if (toStart.length > 0) {
        log.info(`[processQueue] Procesadas ${toStart.length} descargas de ${queuedCount} en cola`);
      }
    } catch (error) {
  ...
  async addDownload(download: AddDownloadPayload): Promise<Snapshot> {
    ...
    if (existingDownload && !download.forceOverwrite) {
      ...
      if (needsConfirmation) {
        ...
        return this.stateStore.getSnapshotLimited(SNAPSHOT_LIMIT);
      }
    }
    ...
    try {
      ...
      if (!preparedDownload.forceOverwrite && preparedDownload.savePath) {
        try {
          ...
          log.debug?.(
            `[processDownload] Archivo de destino aún no existe para descarga ${downloadId}, procediendo con la descarga`
          );
        }
      }
      ...
      this._downloadIdsPendingStart.delete(downloadId);
    }
  }
  ...
  async pauseAll(): Promise<Snapshot> {
    const snapshot = this.stateStore.getSnapshotLimited(SNAPSHOT_LIMIT_BULK);
  ...
  async restartStoppedWithOverwrite(ids?: number[]): Promise<Snapshot> {
    const candidate = ids
      ? (ids
          .map(id => this.stateStore.getDownload(id))
          .filter(
            (d): d is NonNullable<typeof d> => d != null
          ) as import('./StateStore').Download[])
      : this.stateStore.getSnapshotLimited(SNAPSHOT_LIMIT_BULK).downloads;
  ...
  getSessionMetrics(): Record<string, unknown> {
    ...
    (metrics as Record<string, unknown>).queueDepth = this.stateStore.getSummary().queued;
  ...
  async close(): Promise<void> {
    ...
    const active = this.stateStore.getDownloadsByStateLimited(
      DownloadState.DOWNLOADING,
      SNAPSHOT_LIMIT_BULK
    );
    ...
    this.eventBus.clear(); // Cancela timer de debounce y quita listeners (evita retención)
...
/** Exportación solo de la clase. La instancia en runtime se crea en ipcStateHandlers y se obtiene con getDownloadEngine(). */
