...
/**
 * Almacén en disco de archivos temporales de chunks (userData/temp/chunks/{downloadId}/).
 * Proporciona rutas, creación de directorios, listado, borrado y reconciliación FS vs DB.
 * No es transaccional con StateStore; la DB es la fuente de verdad del estado de cada chunk.
 */
export default class ChunkStore {
  ...
  /**
   * Crea el directorio base (userData/temp/chunks) y comprueba permisos de escritura.
   * @throws Error si no hay permisos o falla la creación.
   */
  ...
  /** Ruta del directorio de chunks de una descarga. */
  getChunkDir(downloadId: number): string {
    return path.join(this.baseTempDir, String(downloadId));
  }

  /** Ruta absoluta del archivo temporal de un chunk (.chunk.{index}). */
  ...
  /** Ruta del archivo staging para merge (mismo directorio que chunks, nombre final + .staging). */
  ...
  /** Crea el directorio de chunks de la descarga (recursive). @returns Ruta del directorio. */
  ...
  /** Lista los chunks presentes en disco para la descarga (ordenados por index). */
  ...
  /** Elimina un archivo de chunk del disco. @returns true si se eliminó; false si ENOENT o error. */
  ...
  /** Elimina todos los chunks de la descarga y el directorio. @returns Número de archivos eliminados. */
  ...
  /**
   * Compara chunks en disco con los registros en DB: detecta huérfanos (en FS sin DB),
   * faltantes (en DB completed sin archivo) y tamaño incorrecto.
   * @param downloadId - ID de la descarga.
   * @param dbChunks - Chunks según StateStore (chunkIndex, startByte, endByte, state).
   * @returns ReconcileResult con orphaned, missing, mismatched, total, inDb.
   */
  ...
  /**
   * Elimina directorios de chunks cuyos downloadId no están en activeDownloadIds (huérfanos).
   * @param activeDownloadIds - IDs de descargas que siguen activas o en cola.
