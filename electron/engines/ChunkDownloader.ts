...
/**
 * Aborta y limpia todos los chunks activos de una descarga (request, response, streams, timers).
 * @param engine - Referencia al motor (chunkManager).
 * @param downloadId - ID de la descarga cuyos chunks deben abortarse.
 */
...
/**
 * Fusiona los archivos temporales de chunks en el archivo final vía worker y actualiza estado a verifying.
 * Requiere que todos los chunks estén en estado completed. Emite mergeStarted y, al terminar, transition a verifying.
 *
 * @param engine - Referencia al motor (stateStore, eventBus, _runWorker para merge).
 * @param downloadId - ID de la descarga.
 * @param isRetry - Si true, la descarga ya estaba en MERGING (reintento de merge).
 * @returns Promesa que se resuelve al completar el merge y verificación (o al cancelar).
 */
export async function mergeChunks(
  ...
  try {
    ...
    // Eliminar staging de un merge previo interrumpido para evitar archivos residuales
    ...
    // Primer intento: reintentar silenciosamente sin emitir FAILED; el estado se mantiene en MERGING para que el retry pueda reutilizarlo y se evita flash de error en la UI.
    if (!isRetry) {
      log.info(
...
/**
 * Guarda checkpoint de integridad (hash de últimos N bytes) para cada chunk en DOWNLOADING (reanudar después).
 * Debe llamarse antes de pausar una descarga fragmentada.
 *
 * @param engine - Referencia al motor (stateStore, chunkStore).
 * @param downloadId - ID de la descarga.
 * @returns Promesa que se resuelve cuando se guardaron los checkpoints de todos los chunks en DOWNLOADING.
 */
...
/**
 * Inicia o reanuda una descarga fragmentada: crea/actualiza chunks en StateStore y ChunkStore,
 * descarga en paralelo con downloadChunks (respetando maxConcurrentChunks), y al completar
 * todos los chunks ejecuta merge y verificación. Soporta reanudación desde checkpoints.
 *
 * @param engine - Referencia al motor (stateStore, chunkStore, eventBus, sessionManager, _unregisterDownloadHost).
 * @param download - id, totalBytes, savePath, url; opcional forceOverwrite, startedAt.
 * @returns Promesa que se resuelve cuando la descarga fragmentada termina (merge y verificación) o se cancela.
 */
...
/**
 * Descarga en paralelo la lista de chunks de una descarga, respetando maxConcurrentChunks.
 * Cada chunk se descarga con downloadSingleChunk; al completar todos se llama a mergeChunks.
 * Gestiona reintentos por chunk y emite progreso/eventos vía eventBus.
 *
 * @param engine - Referencia al motor (stateStore, chunkStore, eventBus, chunkManager, etc.).
 * @param downloadId - ID de la descarga.
 * @param chunks - Rangos byte start/end por chunk.
 * @param sessionId - ID de sesión para invalidar si la descarga se cancela/pausa.
 * @returns Promesa que se resuelve al completar todos los chunks o al invalidar la sesión.
 */
export async function downloadChunks(
  ...
  while (true) {
    ...
    if (status === 'fulfilled') {
      // Fusión incremental: escribir al staging a medida que los chunks completan en orden
      ...
      if (
        ...
        try {
          ...
          log.error(`[O13] Error en fusión incremental descarga ${downloadId}:`, incErr);
          engine.removeIncrementalMergeSession(downloadId);
          try {
...
/**
 * Descarga un único chunk por HTTP Range (request con Range header), escribe en archivo temporal
 * y actualiza estado en StateStore. Si sessionId se pasa y ya no es la sesión actual, no hace nada.
 *
 * @param engine - Referencia al motor (stateStore, chunkStore, eventBus, chunkManager, sessionManager, etc.).
 * @param downloadId - ID de la descarga.
 * @param chunkIndex - Índice del chunk.
 * @param chunk - Rango { start, end }; si null se obtiene de stateStore.getChunks.
 * @param sessionId - Si se pasa, se valida antes de iniciar (evitar progreso tras cancelar).
 * @returns Promesa que se resuelve al terminar la descarga del chunk (éxito o fallo).
 */
