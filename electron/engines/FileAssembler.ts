...
/**
 * Ensamblador de chunks: concatena archivos temporales en el archivo final (staging + rename atómico)
 * o sesión incremental (append por chunk en orden). Valida espacio antes de escribir.
 */
export default class FileAssembler {
   ...
   * @param downloadId - ID de la descarga (para rutas de ChunkStore).
   * @param finalPath - Ruta absoluta del archivo final.
   * @param expectedSize - Tamaño esperado en bytes (para reserva y validación).
   * @param chunkCount - Número total de chunks (para saber cuándo finalizar).
   * @returns Sesión con appendChunk y finalize (rename staging → finalPath).
   */
  ...
  /**
   * Concatena los chunks en orden en un archivo staging y lo renombra a finalPath (atómico).
   * Doble búfer: pre-lectura del siguiente bloque mientras se escribe el actual. Valida espacio y borra chunks al terminar.
   *
   * @param downloadId - ID de la descarga (para staging path y limpieza).
   * @param chunks - Lista de chunks con index, path y opcionalmente startByte/endByte/size.
   * @param finalPath - Ruta absoluta del archivo final.
   * @param expectedSize - Tamaño esperado (validación de espacio).
   * @param onProgress - Callback opcional (progreso 0–1, bytes procesados).
   * @param forceOverwrite - Si true, sobrescribe finalPath si existe.
