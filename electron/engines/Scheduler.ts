...
/**
 * Planificador de la cola: limita descargas simultáneas (global y por host), aplica rate limit
 * por host y ordena candidatos por prioridad efectiva (aging, SJF, penalización por reintentos).
 */
export default class Scheduler {
  ...
  /**
   * Tamaño efectivo de una descarga para ordenación SJF; si no hay totalBytes usa valor por defecto.
   * @param download - Entrada tipo cola con total_bytes o totalBytes.
   * @returns Tamaño en bytes (o sjfDefaultSizeBytes si no está definido).
   */
  ...
  /**
   * Comparación de tamaños para SJF; si la diferencia relativa es menor que la tolerancia, devuelve 0.
   * @param sizeA - Tamaño en bytes del primer ítem.
   * @param sizeB - Tamaño en bytes del segundo ítem.
   * @returns Negativo si A < B, positivo si A > B, 0 si equivalentes o SJF desactivado.
   */
  ...
  /**
   * Prioridad efectiva para ordenar la cola: prioridad base + aging − penalización por reintentos.
   * @param download - Entrada con priority, created_at/createdAt, retry_count/retryCount.
   * @param now - Timestamp actual (ms) para calcular tiempo en cola.
   * @returns Valor numérico; mayor = mayor prioridad.
   */
  ...
  /**
   * Comprueba si se puede iniciar una descarga más (límite global, por host y rate limit).
   * @param currentActiveCount - Número de descargas actualmente activas.
   * @param host - Hostname de la URL (opcional); si se pasa, se aplica límite por host y rate limit.
   * @returns CanStartResult con canStart, reason opcional, slotsAvailable, hostLimit y rateLimited.
   */
  ...
  /** Registra una descarga como activa para el host dado (conteo y rate limiter). */
  ...
  /** Da de baja una descarga del conteo activo del host. */
   ...
   *
   * @param queuedDownloads - Lista de descargas en estado queued.
   * @param slotsAvailable - Número de slots a rellenar.
   * @param currentActiveCount - Conteo actual de activas (opcional); si no se pasa se usa getActiveCount().
   * @returns Subconjunto de queuedDownloads listo para arrancar (respeta límites por host y rate limit).
   */
  ...
  /** Extrae el hostname de una URL; devuelve null si la URL no es válida. */
  ...
  /** Número total de descargas registradas como activas (suma por host). */
  ...
  /** Libera recursos de los rate limiters por host (llamar al cerrar el motor). */
  cleanup(): void {
