...
/** Objeto tipo IncomingMessage para la respuesta HTTP de un chunk (statusCode, headers, on, destroy, pause, resume). */
...
/**
 * Gestiona el flujo de datos de una respuesta HTTP de chunk: backpressure, idle timeout, validación de completitud, escritura a tempFile, cleanup.
 * Resuelve o rechaza la promesa según el resultado; en sesión inválida o estado no DOWNLOADING hace cleanup y resolve sin error.
 *
 * @param engine - Referencia al ChunkEngine (stateStore, chunkStore, eventBus, chunkManager, etc.).
 * @param downloadId - ID de la descarga.
 * @param chunkIndex - Índice del chunk.
 * @param chunk - Rango { start, end } del chunk.
 * @param response - Respuesta HTTP (ChunkResponseLike).
 * @param request - Request HTTP (para abort en cleanup).
 * @param actualStartByte - Byte inicial real (puede diferir si reanudación).
 * @param downloadedBytes - Bytes ya descargados del chunk (para reanudación).
 * @param resolve - Callback de éxito.
 * @param reject - Callback de error.
 * @param sessionId - ID de sesión actual (opcional); si no coincide con el del engine se aborta.
 */
