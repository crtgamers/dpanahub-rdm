...
export class BufferPool {
   ...
   *
   * @returns Buffer de tamaño bufferSize (del pool o recién alocado).
   */
  acquire(): Buffer {
   ...
   * Si requestedSize > bufferSize, retorna un buffer nuevo del tamaño solicitado (no pooleable).
   *
   * @param requestedSize - Bytes que se van a usar; si > bufferSize se aloca uno ad-hoc.
   * @returns Buffer de tamaño bufferSize o requestedSize (si mayor).
   */
  ...
  /**
   * Devuelve un buffer al pool. Si el pool está lleno, el buffer se descarta (GC).
   * Solo acepta buffers de exactamente bufferSize bytes; otros se ignoran.
   *
   * @param buffer - Buffer a devolver (debe tener length === bufferSize para reciclar).
   */
   ...
   * @returns BufferPoolStats (name, bufferSize, availableCount, hitRate, totalReuses, etc.).
   */
