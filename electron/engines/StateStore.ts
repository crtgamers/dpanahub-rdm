...
interface StateStoreStatements {
  ...
  getByStateLimited: { all: (_state: string, _limit: number) => Row[] };
...
/**
 * Fuente de verdad del estado de la cola de descargas.
 *
 * Persistencia transaccional en SQLite (WAL). Expone: addDownload, updateDownload,
 * transitionState (con máquina de estados), getSnapshot (completo o incremental),
 * getChunks, getAttempts, hooks de transición y batch de progreso para no saturar la DB.
 */
export class StateStore {
  ...
  /**
   * Inicializa el almacén: crea el directorio si no existe, abre *-state.db con WAL,
   * ejecuta el schema, prepara statements y recupera descargas en estado activo a queued.
   *
   * @returns true si la inicialización fue correcta; false en caso de error (log incluido).
   */
  initialize(): boolean {
    ...
    try {
      ...
      // cache_size negativo está en bloques de 1KB: -16384 ≈ 16MB de caché
      this._db.pragma('cache_size = -16384');
  ...
  private _prepareStatements(): void {
    ...
    this.statements = {
      ...
      getByStateLimited: db.prepare(
        `SELECT * FROM downloads WHERE state = ?
         ORDER BY priority DESC, queue_position ASC, created_at ASC
         LIMIT ?`
      ) as StateStoreStatements['getByStateLimited'],
  ...
  /**
   * Devuelve un snapshot del estado actual de la cola (descargas, resumen, stateVersion).
   * Si minVersion no es null y stateVersion <= minVersion, devuelve snapshot sin cambios (hasChanges: false).
   *
   * @param minVersion - Si se pasa y la versión actual es menor o igual, el snapshot indica que no hay cambios.
   * @returns Snapshot con stateVersion, downloads, summary y hasChanges.
   */
  ...
  /**
   * Snapshot limitado para UI: devuelve solo las primeras `limit` descargas ordenadas,
   * junto con el summary calculado directamente desde la DB.
   *
   * No se usa para operaciones internas del motor (pauseAll, clear, etc.) para no
   * perder visibilidad sobre descargas fuera de la ventana visible.
   */
  getSnapshotLimited(limit: number): Snapshot {
    if (!this._db || !this.statements) {
      return {
        stateVersion: 1,
        downloads: [],
        hasChanges: false,
    ...
    const stateVersion = this.getStateVersion();
    const summary = this._calculateSummaryFromDb();
    ...
    const downloads = rows.map((row: Row) => this._rowToDownload(row));
  ...
  /**
   * Devuelve el resumen de la cola (conteos por estado) sin cargar descargas en memoria.
   * Útil para métricas y UI que solo necesitan totales.
   */
  getSummary(): SnapshotSummary {
    return this._calculateSummaryFromDb();
  }
  ...
  /**
   * Inserta una nueva descarga en la cola (estado inicial queued o paused según startPaused).
   * Respeta maxQueueSize salvo que skipQueueLimit sea true. Dispara el trigger de state_version.
   *
   * @param download - Datos mínimos: id, title; opcionales url, savePath, priority, startPaused, etc.
   * @returns Snapshot actualizado tras la inserción.
   * @throws Error si la cola está llena y no se usa skipQueueLimit.
   */
  ...
  /**
   * Cambia el estado de una descarga si la transición está permitida por la máquina de estados.
   * Ejecuta hooks onExit/onEnter si están configurados.
   *
   * @param id - ID de la descarga.
   * @param newState - Nuevo estado (queued, starting, downloading, paused, merging, verifying, completed, failed, cancelled).
   * @param oldState - Estado anterior (opcional; si no se pasa se lee de la DB).
   * @returns true si la transición se aplicó; false si la descarga no existe o la transición es inválida.
   */
  ...
  /**
   * Devuelve hasta `limit` descargas en el estado dado (evita cargar todas en memoria).
   * Orden: priority DESC, queue_position ASC, created_at ASC.
   */
  getDownloadsByStateLimited(state: string, limit: number): Download[] {
    if (!this.statements || limit <= 0) return [];
    return this.statements.getByStateLimited
      .all(state, limit)
      .map((row: Row) => this._rowToDownload(row));
  }

  /**
   * Actualiza campos de una descarga (estado, progreso, bytes, errores, checkpoints, etc.).
   * Si updates.state está presente y es distinto del actual, se valida con la máquina de estados.
   *
   * @param id - ID de la descarga.
   * @param updates - Campos a actualizar (state, progress, downloadedBytes, lastError, partialTailHash, etc.).
   * @returns true si se aplicó al menos un cambio; false si no existe la descarga o la transición es inválida.
   */
  ...
  /**
   * Quita de la lista solo las descargas en estado completed (no borra archivos en disco).
   * Útil para hacer espacio en la UI antes de agregar muchas descargas manuales.
   */
  clearCompletedFromList(): number {
    if (!this._db) return 0;
    try {
      const result = this._db.prepare("DELETE FROM downloads WHERE state = 'completed'").run();
      if (result.changes > 0) {
        this._db.prepare('UPDATE state_version SET version = -(ABS(version) + 1)').run();
        this._db.prepare('UPDATE state_version SET version = -version').run();
      }
      log.info(`Eliminadas ${result.changes} descargas completadas de la lista`);
      return result.changes;
    } catch (error) {
      log.error('Error eliminando completadas de la lista:', error);
      return 0;
    }
  }
  ...
  close(): void {
    ...
    this.progressBatch.clear(); // Liberar referencias explícitamente
    if (this._db) {
