 ...
 * - guardValidation: devuelve { success: false, error } si la validación falla, o null si es válida;
 *   evita repetir el patrón if (!validation.valid) return { success: false, error: validation.error }.
...
/**
 * Si la validación falla, devuelve el objeto de error para respuestas IPC; si es válida, devuelve null.
 * Útil para evitar repetir "if (!validation.valid) return { success: false, error: validation.error }" en handlers.
 *
 * @param validation - Objeto con valid (boolean) y opcionalmente error (string).
 * @returns { success: false, error: string } si !validation.valid; null en caso contrario.
 */
export function guardValidation(validation: {
  valid: boolean;
  error?: string;
