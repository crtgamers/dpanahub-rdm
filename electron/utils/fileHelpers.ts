/**
 * Utilidades para operaciones con archivos, sanitización de nombres, espacio en disco y JSON en config.
 *
 * Expone: sanitizeFilename, sanitizePath, checkFileExists, readJSONFile, writeJSONFile,
 * ensureDirectoryExists, hasWritePermission, getAvailableDiskSpace, formatBytes, validateDiskSpace, safeUnlink.
 *
 * @module fileHelpers
 */
...
/** Resultado de checkFileExists: si existe, tamaños y si la diferencia está dentro del margen configurado. */
...
/** Sanitiza un nombre de archivo: reemplaza caracteres prohibidos, evita nombres reservados Windows y limita longitud. */
...
/** Sanitiza cada segmento de una ruta con sanitizeFilename y vuelve a unir con path.sep. */
...
/**
 * Comprueba si el archivo existe y opcionalmente si su tamaño es similar al esperado (según config.files.sizeMarginBytes).
 *
 * @param filePath - Ruta absoluta al archivo.
 * @param expectedSize - Tamaño esperado en bytes.
 * @returns FileCheckResult con exists, existingSize, expectedSize, sizeDifference, similarSize.
 */
 ...
 *
 * @param filename - Nombre del archivo (ej. settings.json) dentro de config.paths.configPath.
 * @returns Objeto parseado o null si no existe o hay error de lectura/parseo.
 */
 ...
 *
 * @param filename - Nombre del archivo dentro de config.paths.configPath.
 * @param data - Objeto a serializar (JSON.stringify con indentación 2).
 * @returns true si se escribió correctamente; false en error.
 */
...
/** Crea el directorio (y padres) si no existe. Síncrono. @returns true si existe o se creó; false en error. */
...
/** Comprueba si el proceso tiene permisos de escritura en la ruta (fs.accessSync W_OK). */
...
/** Máximo de entradas en caché (evita crecimiento indefinido con muchas rutas/unidades). */
const DISK_SPACE_CACHE_MAX_SIZE = 200;

const diskSpaceCache = new Map<string, { bytes: number; ts: number }>();

function evictDiskSpaceCacheIfNeeded(): void {
  if (diskSpaceCache.size < DISK_SPACE_CACHE_MAX_SIZE) return;
  let oldestKey: string | null = null;
  let oldestTs = Infinity;
  for (const [key, entry] of diskSpaceCache) {
    if (entry.ts < oldestTs) {
      oldestTs = entry.ts;
      oldestKey = key;
    }
  }
  if (oldestKey != null) diskSpaceCache.delete(oldestKey);
}
...
/** Formatea bytes a string legible (Bytes, KB, MB, GB, TB). */
...
/** Resultado de validateDiskSpace: valid, mensaje error opcional, warning si no se pudo verificar, available/required en bytes. */
...
/**
 * Valida que haya espacio suficiente en disco (async). Requiere ~110% del tamaño esperado.
 *
 * @param filePath - Ruta de destino (archivo o directorio); se usa para resolver la unidad.
 * @param expectedSize - Tamaño esperado en bytes.
 * @returns ValidateDiskSpaceResult (valid, error opcional, warning si no se pudo leer espacio).
 */
...
/** Elimina un archivo de forma asíncrona; si falla por EBUSY/EPERM reintenta una vez tras retryDelay ms. */
