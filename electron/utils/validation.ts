...
import { validateDownloadPathSync } from './downloadPath';
...
/**
 * Valida que la ruta de descarga sea segura: no raíz del sistema, no directorios críticos del SO,
 * normalizada y sin path traversal. Usa el módulo downloadPath para listas de rutas bloqueadas.
 * Las rutas "sensibles" se consideran válidas aquí; el frontend puede mostrar advertencia vía IPC.
 *
...
export function validateAndSanitizeDownloadPath(downloadPath: string): PathValidationResult {
  ...
  const result = validateDownloadPathSync(downloadPath);
  if (!result.valid || result.blockReason === 'critical') {
    return { valid: false, error: result.error ?? 'Ruta de descarga no válida' };
  }
  return { valid: true, path: result.path };
}

/**
 * Sanitiza un nombre de archivo: elimina caracteres prohibidos en el SO, trunca a longitud máxima
 * y devuelve 'unnamed' si el resultado queda vacío.
 *
 * @param fileName - Nombre de archivo a sanitizar (se usa 'unnamed' si no es string).
 * @returns Nombre seguro para usar en rutas de disco.
 */
...
/**
 * Normaliza prioridad de descarga a 1 (baja), 2 (normal) o 3 (alta).
 * Acepta número 1–3 o string 'low'|'normal'|'high'.
 *
 * @param priority - Prioridad en número (1–3) o string ('low', 'normal', 'high').
 * @returns 1, 2 o 3; por defecto 2 si el valor no es reconocido.
 */
