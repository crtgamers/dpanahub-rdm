/**
 * Implementación del patrón Circuit Breaker para evitar saturar un recurso ante fallos repetidos.
 *
 * Estados: CLOSED (normal), OPEN (rechaza requests hasta timeout), HALF_OPEN (prueba).
 * Opciones: failureThreshold, successThreshold, timeout, resetTimeout, onStateChange, shouldOpen, shouldClose.
 * Expone: execute(operation, fallback), reset(), getState(), isOpen/isClosed/isHalfOpen, destroy().
 *
...
export class CircuitBreaker {
  ...
  /**
   * Crea un circuit breaker con umbrales y timeouts opcionales.
   *
   * @param options - name, failureThreshold, successThreshold, timeout, resetTimeout, onStateChange, shouldOpen, shouldClose.
   */
  ...
  /**
   * Ejecuta la operación si el circuito lo permite; si está OPEN devuelve fallback (o resultado de invocar fallback si es función).
   *
   * @param operation - Función async a ejecutar.
   * @param fallback - Valor o función a devolver cuando el circuito está OPEN.
   * @returns Resultado de operation o fallback.
   */
  async execute<T>(
    operation: () => Promise<T>,
  ...
  /** Fuerza transición a CLOSED y resetea contadores. */
  reset(): void {
  ...
  /** Devuelve estado actual, contadores y stats (totalRequests, totalSuccesses, etc.). */
  getState(): {
    state: CircuitStateType;
    failureCount: number;
    successCount: number;
  ...
  /** true si el circuito está OPEN (rechazando requests). */
  isOpen(): boolean {
    return this.state === CircuitState.OPEN;
  }

  /** true si el circuito está CLOSED (operación normal). */
  isClosed(): boolean {
    return this.state === CircuitState.CLOSED;
  }

  /** true si el circuito está HALF_OPEN (probando). */
  isHalfOpen(): boolean {
    return this.state === CircuitState.HALF_OPEN;
  }

  /** Detiene el intervalo de reset periódico. Llamar al desechar el breaker. */
  destroy(): void {
    if (this.resetInterval) {
      clearInterval(this.resetInterval);
