...
/**
 * Normaliza un tipo crudo del catálogo (Directory → folder, File → file).
 * @param type - Tipo tal como viene de la BD (ej. 'Directory', 'File').
 * @returns 'folder' | 'file' | valor en minúsculas.
 */
...
/** Formatea bytes en unidad legible (B / KiB / MiB / GiB). Devuelve '-' si no es número válido. */
 ...
 *
 * @param item - Nodo crudo (id, name, title, type, size_bytes, etc.).
 * @param pathData - full_path y parent_path del nodo (o undefined).
 * @returns Nodo con name, title, displayTitle, breadcrumbPath, fullPath, type, size normalizados.
 */
 ...
 *
 * @param item - Nodo crudo.
 * @param nodeMap - Mapa id -> { parent_id, name } para reconstruir rutas.
 * @returns Nodo con name, title, displayTitle, breadcrumbPath, fullPath, type, size normalizados.
 */
