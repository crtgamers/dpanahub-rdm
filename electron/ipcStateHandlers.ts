...
import config, { MASS_DOWNLOAD_MAX_FILES } from './config';
...
import chunkProgressCache from './engines/ChunkProgressAggregator';
import circuitBreakerManager from './engines/CircuitBreakerManager';
...
/** Quita todos los listeners registrados en el EventBus y los timers de throttling de progreso. */
...
/** Rate limit para get-download-state (máx. 2–3 req/s por ventana) para evitar picos de CPU en main. */
 ...
 *
 * @param _mainWindow - Ventana principal (opcional; no se usa en esta función pero mantiene firma para compatibilidad).
 */
...
/**
 * Registra todos los handlers IPC del motor de descargas (get-download-state, add-download,
 * pause/resume/cancel/delete, download-folder, apply-settings, test de conexión, etc.).
 * Requiere mainWindow para contexto de ventana en algunos handlers.
 *
 * @param mainWindow - Ventana principal de Electron (para enviar eventos al renderer).
 */
export function registerStateHandlers(mainWindow: Electron.BrowserWindow): void {
  ...
  ipcMain.handle('ensure-download-engine', async () => {
    try {
      if (!downloadEngine || !downloadEngine.isInitialized) {
        await initializeDownloadEngine(mainWindow);
      }
      const initialized = !!downloadEngine && downloadEngine.isInitialized;
      return { success: initialized, data: { initialized } };
    } catch (error) {
      log.error('Error asegurando DownloadEngine:', error);
      return {
        success: false,
        error: (error as Error).message || 'No se pudo inicializar el motor de descargas',
      };
    }
  });
  ...
  ipcMain.handle('get-download-state', async (_event, minVersion: number | null = null) => {
    try {
      ...
      // Snapshot completo (inicial o fallback), limitado para UI
      const stateStore = downloadEngine.stateStore;
      const snapshot = stateStore.getSnapshotLimited(MAX_SNAPSHOT_DOWNLOADS);
      lastSnapshotServedTimestamp = Date.now();

      const totalCount = snapshot.summary.total;
      if (!totalCount || totalCount <= snapshot.downloads.length) {
  ...
  ipcMain.handle('download-folder', async (_event, ...args) => {
    ...
    if (err) {
      log.error('Parámetros de descarga de carpeta inválidos:', err.error);
      return err;
    }
    ...
    try {
      const filesResult = await database.getAllFilesInFolder(
        validatedParams.folderId,
        MASS_DOWNLOAD_MAX_FILES
      );
      ...
      const FOLDER_CLEAR_COMPLETED_THRESHOLD = 500;
      if (files.length > FOLDER_CLEAR_COMPLETED_THRESHOLD) {
        const stateStore = downloadEngine.stateStore;
        if (stateStore) {
          const cleared = stateStore.clearCompletedFromList();
          if (cleared > 0) {
            lastProgressSentByDownload.clear();
            progressBatchBuffer.clear();
            chunkProgressCache.clearAll();
            eventBus.emitStateChanged(stateStore.getStateVersion());
            log.info(
              `Descarga de carpeta/romset (${files.length} archivos): ${cleared} completadas eliminadas de la lista antes de agregar`
            );
          }
        }
      }
  ...
  ipcMain.handle(
    ...
    createHandler('delete-download-state', async (_event, ...args) => {
      ...
      // Limpiar throttle de progreso y caché de chunks para esta descarga (liberar RAM)
      clearProgressThrottleForDownload(id);
      chunkProgressCache.clear(id);
  ...
  ipcMain.handle(
    ...
    createHandler('clear-downloads-state', async () => {
      ...
      // Limpiar throttles, buffer de progreso y caché de chunks para liberar RAM
      lastProgressSentByDownload.clear();
      progressBatchBuffer.clear();
      chunkProgressCache.clearAll();
      eventBus.emitStateChanged(stateStore.getStateVersion());
  ...
  ipcMain.handle(
    ...
    createHandler('clear-completed-from-list', async () => {
      if (!downloadRateLimiter.isAllowed('ipc-handler')) {
        throw new Error('Demasiadas solicitudes, intente más tarde');
      }
      if (!downloadEngine) {
      ...
      const count = stateStore.clearCompletedFromList();
      lastProgressSentByDownload.clear();
      progressBatchBuffer.clear();
      chunkProgressCache.clearAll();
      eventBus.emitStateChanged(stateStore.getStateVersion());
  ...
  ipcMain.handle(
    ...
    createHandlerBase(
      ...
      async (_event, ...args) => {
        ...
        // Motor puede no existir aún al arranque (se crea al seleccionar fuente); no es error.
        if (!downloadEngine?.isInitialized) {
          if (log.debug)
            log.debug(
              '[apply-download-settings] Motor aún no disponible, se aplicará al inicializarlo'
            );
          return { success: true };
        }
      ...
      { wrapSuccess: true, log, app, defaultErrorMessage: 'Error desconocido' }
    )
  );
...
/**
 * Quita todos los handlers IPC registrados por registerStateHandlers.
 * No cierra el motor; para cerrar usar closeDownloadEngine().
 */
...
/**
 * Cierra el DownloadEngine (cola, workers, StateStore), libera la referencia y quita los listeners del EventBus.
 * Debe llamarse antes de salir de la aplicación para un cierre ordenado.
 */
export async function closeDownloadEngine(): Promise<void> {
  ...
  circuitBreakerManager.destroyAll(); // Detiene intervalos de reset (evita retención)
  cleanupEventListeners();
}

/**
 * Devuelve la instancia actual del motor de descargas.
 * @returns DownloadEngine o null si no está inicializado o ya se cerró.
 */
