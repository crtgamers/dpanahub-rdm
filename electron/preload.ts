...
const validEventChannels = [
  ...
  'update-error',
  'catalog-database-download-progress',
] as const;
...
export interface PreloadAPI {
  ...
  getDatabaseStatus: (_source: 'myrient' | 'lolroms') => Promise<unknown>;
  downloadCatalogDatabase: (_source: 'myrient' | 'lolroms') => Promise<unknown>;
  ...
  ensureDownloadEngine: () => Promise<unknown>;
  ...
  clearCompletedFromList: () => Promise<unknown>;
  ...
  getDefaultDownloadDir: (_createIfMissing?: boolean) => Promise<unknown>;
  validateDownloadPath: (_filePath: string) => Promise<unknown>;
  ...
  getPendingUpdateVersion: () => Promise<unknown>;
  isUpdaterEnabled: () => Promise<unknown>;
  romsetBuilderSummary: (_params: { folderIds: number[] }) => Promise<unknown>;
...
const api: PreloadAPI = {
  ...
  getDatabaseStatus: (source: 'myrient' | 'lolroms') => safeInvoke('get-database-status', source),
  downloadCatalogDatabase: (source: 'myrient' | 'lolroms') =>
    safeInvoke('download-catalog-database', source),
  onCatalogDatabaseDownloadProgress: (
    callback: (_payload: {
      source: string;
      percent: number;
    ...
    safeOn('catalog-database-download-progress', (...args: unknown[]) =>
      callback(args[0] as { source: string; percent: number; received: number; total: number })
    ),
  ...
  clearNavCache: () => safeInvoke('clear-nav-cache'),
  ...
  ensureDownloadEngine: () => safeInvoke('ensure-download-engine'),
  ...
  clearCompletedFromList: () => safeInvoke('clear-completed-from-list'),
  ...
  getDefaultDownloadDir: (createIfMissing?: boolean) =>
    safeInvoke('get-default-download-dir', createIfMissing),
  validateDownloadPath: (filePath: string) => safeInvoke('validate-download-path', filePath),
  ...
  getPendingUpdateVersion: () => safeInvoke('get-pending-update-version'),
  isUpdaterEnabled: () => safeInvoke('is-updater-enabled'),

  romsetBuilderSummary: params => safeInvoke('romset-builder-summary', params),
...
// Log de diagnóstico solo en desarrollo (evitar ruido en producción)
