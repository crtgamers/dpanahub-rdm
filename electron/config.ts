...
/** Límite alto de archivos para descarga masiva (carpeta completa / Romset Builder); no aplica límite estándar de cola. */
export const MASS_DOWNLOAD_MAX_FILES = 100000;
...
const config: AppConfig = {
  ...
  rateLimiting: {
    ...
    /** Límite para get-download-state (evitar picos de CPU en main si el renderer pide estado muy a menudo). */
    getDownloadState: {
      maxRequests: 3,
      windowMs: 1000,
    },
  ...
  workers: {
    poolSize: 2, // Valor por defecto cuando maxWorkers no está definido (fallback para compatibilidad)
  ...
  paths: {
    ...
    pleasuredomeDbPath: app.isPackaged
      ? path.join(process.resourcesPath, 'pleasuredome_data.db')
      : path.join(process.cwd(), 'resources', 'pleasuredome_data.db'),
    pleasuredomeCompressed7zPath: app.isPackaged
      ? path.join(process.resourcesPath, 'pleasuredome_data.7z')
      : path.join(process.cwd(), 'resources', 'pleasuredome_data.7z'),
    myabandonwareDbPath: app.isPackaged
      ? path.join(process.resourcesPath, 'myabandonware_data.db')
      : path.join(process.cwd(), 'resources', 'myabandonware_data.db'),
    myabandonwareCompressed7zPath: app.isPackaged
      ? path.join(process.resourcesPath, 'myabandonware_data.7z')
      : path.join(process.cwd(), 'resources', 'myabandonware_data.7z'),
  ...
  security: {
    allowedHosts: Object.freeze([
      ...
      'd2.xp.myabandonware.com', // CDN de descargas de My Abandonware
      // Pleasure Dome (y posibles fuentes) sirven archivos desde GitHub
      'github.com',
...
/**
 * Configuración por defecto del proceso main (runtime).
 * Objeto con: network, circuitBreaker, downloads, ui, rateLimiting, database, workers,
 * retryProfiles, bufferPool, timing, paths, security, files, window.
 * Los valores que el usuario cambia desde la UI se persisten en JSON bajo configPath, no aquí.
 */
