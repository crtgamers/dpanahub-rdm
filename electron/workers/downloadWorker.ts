/**
 * Worker thread para verificación de hash (SHA-256) y ensamblaje de chunks en archivo final.
 *
 * Protocolo: mensajes type PING (responde PONG), VERIFY_HASH (filePath; responde hash y PROGRESS opcional),
 * ASSEMBLE (downloadId, chunks, finalPath, expectedSize, forceOverwrite; responde SUCCESS con result o ERROR).
 *
...
/** Chunk a ensamblar: path al archivo temporal, index de orden, y opcionalmente startByte/endByte. */
...
/**
 * Calcula SHA-256 del archivo; opcionalmente envía mensajes PROGRESS por parentPort.
 *
 * @param filePath - Ruta absoluta al archivo.
 * @param reportProgress - Si true, envía { type: 'PROGRESS', progress, bytesProcessed } durante la lectura.
 * @returns Hash hexadecimal del archivo.
 */
...
/**
 * Concatena los chunks en orden en finalPath (vía archivo .staging + rename), elimina chunks y opcionalmente el directorio.
 * Envía PROGRESS por parentPort durante la escritura.
 *
 * @param _downloadId - ID de descarga (reservado para logs).
 * @param chunks - Lista de chunks (path, index, startByte?, endByte?); se ordenan por index.
 * @param finalPath - Ruta del archivo final.
 * @param expectedSize - Tamaño esperado; se valida al final.
 * @param forceOverwrite - Si true, elimina finalPath antes del rename cuando ya existe.
 * @returns Objeto con success, finalPath, bytesProcessed, duration, chunksDeleted o error.
 */
async function assembleFile(
  ...
  try {
    ...
    // Doble búfer: pre-lectura del siguiente bloque mientras se escribe el actual
...
/** Mensaje entrante: type PING | VERIFY_HASH (filePath) | ASSEMBLE (downloadId, chunks, finalPath, expectedSize, forceOverwrite). */
...
parentPort!.on('message', async (task: TaskMessage) => {
  // Comprobación de salud: responder PING con PONG inmediatamente
  if (task.type === 'PING') {
