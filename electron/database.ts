/**
 * Módulo de acceso al catálogo (Myrient, LoLROMs, PleasureDome, My Abandonware — SQLite).
 *
 * Responsabilidades:
 * - Conexión read-only a las bases de datos de catálogo (tras crear índices y element_paths si aplica).
 ...
 *
 * Soporta múltiples fuentes: Myrient, LoLROMs (tabla content), PleasureDome y My Abandonware (tabla elements).
 *
 * @module database
 */
...
/**
 * Cerrojo por ruta: solo una extracción .7z → .db a la vez por dbPath.
 * Evita que dos llamadas concurrentes a loadDatabase disparen dos 7-Zip y fallen por archivo en uso.
 */
...
class DatabaseService {
  ...
  // Caché de navegación: TTL corto y tamaño moderado para evitar crecer en exceso
  private _navCacheTTL = 20 * 1000;
  private _navCacheMaxSize = 100;
  ...
  /**
   * Estado de la base de datos para una fuente sin cargarla.
   * - 'operational': existe el .db (descomprimido y listo para usar).
   * - 'compressed': no existe .db pero sí el .7z.
   * - 'none': no existe ni .db ni .7z.
   */
  async getDatabaseStatus(source: CatalogSource): Promise<'none' | 'compressed' | 'operational'> {
    const { dbPath, compressed7zPath } = this._getPathsForSource(source);
    const [dbExists, archiveExists] = await Promise.all([
      fileExists(dbPath),
      fileExists(compressed7zPath),
    ]);
    if (dbExists) return 'operational';
    if (archiveExists) return 'compressed';
    return 'none';
  }
  ...
  private _getPathsForSource(source: CatalogSource): {
    ...
    const p = config.paths as {
      ...
      pleasuredomeDbPath?: string;
      pleasuredomeCompressed7zPath?: string;
      myabandonwareDbPath?: string;
    ...
    if (source === 'pleasuredome' && p.pleasuredomeDbPath && p.pleasuredomeCompressed7zPath) {
      return {
        dbPath: p.pleasuredomeDbPath,
        compressed7zPath: p.pleasuredomeCompressed7zPath,
        tableName: 'elements',
      };
    }
    if (source === 'myabandonware' && p.myabandonwareDbPath && p.myabandonwareCompressed7zPath) {
      return {
        dbPath: p.myabandonwareDbPath,
        compressed7zPath: p.myabandonwareCompressed7zPath,
        tableName: 'elements',
      };
    }
    return {
   ...
   *
   * @param source - 'myrient' | 'lolroms' | 'pleasuredome' | 'myabandonware'
   * @returns true si la carga fue exitosa; false en caso contrario (muestra diálogo de error si falta la DB).
   */
  async loadDatabase(source: CatalogSource): Promise<boolean> {
    ...
    let [dbExists, archiveExists] = await Promise.all([
      fileExists(dbPath),
      fileExists(compressed7zPath),
    ]);

    if (!dbExists && archiveExists) {
      const existingLock = extractionLocks.get(dbPath);
      if (existingLock) {
        log.info('Esperando extracción en curso para la misma base de datos...');
        await existingLock;
        dbExists = await fileExists(dbPath);
        if (!dbExists) {
          log.warn('La extracción previa no generó el archivo .db; mostrando error.');
          this._showError(
            'Error de extracción',
            'La extracción anterior no completó correctamente. Vuelve a intentar o descarga el archivo manualmente.'
          );
          return false;
        }
      } else {
        const extractionPromise = (async () => {
          try {
            ...
            extractionLocks.delete(dbPath);
          }
        })();
        extractionLocks.set(dbPath, extractionPromise);
        try {
          await extractionPromise;
          // Reintentos con delay: en Windows el archivo puede no ser visible de inmediato tras 7z.
          const maxFileCheckAttempts = 3;
          const fileCheckDelayMs = 150;
          let dbExistsAfterExtract = false;
          for (let attempt = 1; attempt <= maxFileCheckAttempts; attempt++) {
            dbExistsAfterExtract = await fileExists(dbPath);
            if (dbExistsAfterExtract) break;
            if (attempt < maxFileCheckAttempts) {
              await new Promise(resolve => setTimeout(resolve, fileCheckDelayMs));
            }
          }
          if (!dbExistsAfterExtract) {
            this._showError(
              'Error de extracción',
              `La extracción finalizó pero no se encontró la base de datos en: ${dbPath}`
            );
            return false;
          }
        } catch (error) {
          ...
          this._showError(
            'Error al extraer la base de datos',
    ...
    if (!dbExists && !archiveExists) {
      const dbNames: Record<CatalogSource, string> = {
        myrient: 'myrient_data.db',
        lolroms: 'lolrom_data.db',
        pleasuredome: 'pleasuredome_data.db',
        myabandonware: 'myabandonware_data.db',
      };
      const dbName = dbNames[source];
      this._showError(
        'Error de base de datos',
        `No se encontró el archivo '${dbName}' en: ${dbPath}`
      );
      return false;
    }
    ...
    try {
      ...
      if (!detectedTable) {
        ...
        this._showError(
          ...
          `No se encontró la tabla esperada '${tableName}' o 'elements'. ` +
            `La base de datos debe contener una tabla con columnas: id, parent_id, name, type, url.`
        );
        return false;
      }
      ...
      // Limitar caché de páginas de SQLite para catálogos grandes (~16MB)
      ...
      this._showError(
        ...
        `No se pudo abrir la base de datos: ${(error as Error).message}`
      );
      return false;
    }
  }
  ...
  _ensureElementPathsPopulated(): void {
    ...
    try {
      ...
      // Skip CTE costosa si la tabla ya tiene datos (p. ej. sesión anterior)
      const existingCount =
   ...
   * @param options - Opciones: limit, offset, folderLimit, usePrefix, usePhrase, useOR, signal, includeTotalCount, scopeFolderId, scopeFolderIds.
  ...
  /**
   * Vacía el caché de navegación (getChildren/getNodeInfo). Útil al volver a la raíz
   * para liberar RAM antes del TTL cuando el usuario ya no está navegando por subcarpetas.
   */
  clearNavCache(): void {
   ...
   * @param parentId - ID del nodo padre (carpeta).
   * @param options - limit, offset (por defecto config.defaultChildrenLimit, máx. 10000).
   * @returns GetChildrenResult con success, data (NodeInfo[]), total, error.
   */
   ...
   * @returns Objeto con success, data (array de { id, title, name }), error.
   */
   ...
   * @param nodeId - ID del nodo.
   * @returns Objeto con success, data (fila cruda del nodo), error.
   */
  ...
  /**
   * Devuelve URL, nombre y tamaño de un nodo archivo para iniciar una descarga.
   * @param nodeId - ID del nodo (debe ser tipo file).
   * @returns { url, name, size_bytes } o null si no existe o no tiene URL.
   */
   ...
   * @returns Objeto con id, url, name, size_bytes o null si no hay datos.
   */
   ...
   * @param nodeId - ID del nodo.
   * @returns Array de { name } desde la raíz hasta el padre del nodo (orden ascendente).
   */
   ...
   * @param folderId - ID de la carpeta (debe ser tipo folder).
   * @param maxFilesOverride - Opcional: límite de archivos (p. ej. para descarga masiva de carpeta/romset). Si no se pasa, se usa config.downloads.maxFilesPerFolder.
   * @returns Promesa con success, data (NodeInfo[]), error; respeta maxFilesPerFolder de config o el override.
   */
  async getAllFilesInFolder(
    ...
    try {
      ...
      const maxFilesLimit =
        maxFilesOverride !== undefined && maxFilesOverride > 0
          ? maxFilesOverride
          : ((config.downloads as { maxFilesPerFolder?: number } | undefined)?.maxFilesPerFolder ??
            1000);

      if (this.workerManager.isInitialized) {
        try {
          const workerResult = await this.workerManager.getAllFilesInFolder(
            folderId,
            maxFilesLimit
          );
          if (workerResult.success) {
      ...
      const GET_ALL_FILES_TIMEOUT_MS = 30000;
      return await withTimeout(getAllFilesSync(), GET_ALL_FILES_TIMEOUT_MS, 'getAllFilesInFolder');
    } catch (error) {
      ...
      if (err.message?.includes('tiempo límite')) {
        return {
          success: false,
          error:
            'La operación tardó demasiado. Prueba con una carpeta más pequeña o con menos archivos.',
        };
      }
      return { success: false, error: (error as Error).message };
    }
  ...
  /**
   * Resumen agregado de archivos bajo uno o más folders (recursivo).
   * Devuelve fileCount y totalSizeBytes sin transferir la lista completa de archivos.
   * @param folderIds - IDs de las carpetas (se suman recursivamente todos los archivos descendientes).
   * @returns Objeto con success, data (fileCount, totalSizeBytes), error.
   */
  getFoldersSummary(folderIds: number[]): {
    success: boolean;
    data?: { fileCount: number; totalSizeBytes: number };
    error?: string;
  } {
    if (!this.db || !this.statements) {
      return { success: false, error: 'Base de datos no disponible' };
    ...
    if (!folderIds.length) {
      return { success: true, data: { fileCount: 0, totalSizeBytes: 0 } };
    }
    try {
      const tbl = this._tableName;
      const jsonIds = JSON.stringify(folderIds);
      const stmt = this.db.prepare(`
        WITH RECURSIVE folder_tree AS (
          ...
          UNION ALL
          SELECT n.id, n.type, n.size_bytes
          FROM ${tbl} n
          INNER JOIN folder_tree ft ON n.parent_id = ft.id
        )
        SELECT COUNT(*) as fileCount, COALESCE(SUM(size_bytes), 0) as totalSizeBytes
        FROM folder_tree
        WHERE type = 'file' OR type = 'File'
      `);
      const row = stmt.get(jsonIds) as { fileCount: number; totalSizeBytes: number } | undefined;
      return {
        success: true,
        data: {
      ...
      log.error('Error al obtener resumen de carpetas:', error);
      return { success: false, error: (error as Error).message };
    }
  }
   ...
   * @returns Objeto con success, data (fecha como string o null), error.
   */
  ...
  /**
   * Cierra la conexión a la base de datos, libera statements y vacía la caché de navegación.
   * Debe llamarse antes de cargar otra fuente o al salir de la aplicación.
   * @returns void
   */
  close(): void {
    if (this.db) {
  ...
  _showError(title: string, message: string): void {
    ...
    // No cerrar la aplicación: el usuario puede reintentar o elegir otra fuente.
  }
}
