...
const DB_CONFIGS: DbConfig[] = [
  ...
  {
    dbFile: path.join(resourcesDir, 'pleasuredome_data.db'),
    archivePath: path.join(resourcesDir, 'pleasuredome_data.7z'),
    name: 'pleasuredome_data',
  },
  {
    dbFile: path.join(resourcesDir, 'myabandonware_data.db'),
    archivePath: path.join(resourcesDir, 'myabandonware_data.7z'),
    name: 'myabandonware_data',
  },
];
...
function compressDb(path7za: string, config: DbConfig): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    const proc = spawn(
      path7za,
      ['a', '-t7z', '-mx=7', path.basename(archivePath), path.basename(dbFile)],
      {
        cwd: resourcesDir,
...
async function main(): Promise<void> {
  const [myrient, ...restConfigs] = DB_CONFIGS;
  ...
  for (const cfg of restConfigs) {
    const hasDb = fs.existsSync(cfg.dbFile);
    const has7z = fs.existsSync(cfg.archivePath);
    if (hasDb) hasAnyToCompress = true;
    else if (has7z) {
      const stat = fs.statSync(cfg.archivePath);
      console.log(
        `No hay ${cfg.name}.db; se usará el .7z existente en resources (`,
        (stat.size / 1024 / 1024).toFixed(2),
        'MB ).'
      );
    }
