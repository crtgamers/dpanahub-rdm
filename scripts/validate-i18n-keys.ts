    ...
    for (const file of walkFiles(SRC_DIR, ext)) {
      ...
      lines.forEach((line, i) => {
        ...
        for (const key of keysInLine) {
          ...
          // Ignorar prefijos dinámicos: t('settingsExtra.' + key) → "settingsExtra." no es una key real
          const isDynamicPrefix = key.endsWith('.');
          if (!isDynamicPrefix && !definedKeys.has(key)) {
