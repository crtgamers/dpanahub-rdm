...
exports.default = async function beforePack(context) {
  ...
  // Usar la plataforma para la que se está empaquetando (ej. linux al hacer build --linux),
  // no la plataforma donde corre el build; así se conserva el binario 7za correcto.
  const platformName = context.electronPlatformName || process.platform; // 'linux' | 'darwin' | 'win32'
  ...
  if (fs.existsSync(p7z)) {
    const keepPlatform = platform === 'win32' ? 'win' : platform === 'darwin' ? 'mac' : 'linux';
