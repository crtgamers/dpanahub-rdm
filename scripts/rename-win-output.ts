 ...
 *
 * Renombra dist-electron/win-unpacked a dist-electron/dpanahub-rdm-win.
...
const newName = path.join(outDir, 'dpanahub-rdm-win');
...
function doRename(): void {
  if (fs.existsSync(newName)) {
    console.warn('rename-win-output: ya existe dpanahub-rdm-win, eliminando para reemplazar.');
...
(async () => {
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      doRename();
      console.log('Carpeta renombrada: win-unpacked → dpanahub-rdm-win');
      process.exit(0);
    } catch (err: unknown) {
      ...
      console.error(
        'rename-win-output: no se pudo renombrar win-unpacked → dpanahub-rdm-win.',
        e?.message ?? err
      );
      console.warn(
